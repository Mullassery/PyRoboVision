"""MCP Connector for PyRoboVision - Autonomous Driving Perception"""
...
class PerceptionEngine:
    """Autonomous driving perception with foundation models"""

    def __init__(self):
        self.mcp_connector: Optional[Any] = None

    def start_mcp_connector(self, port: int = 8783) -> str:
        from pyrobovision._mcp_tools import PyRoboVisionMCPHandler, PyRoboVisionMCPTools
        self.mcp_connector = _MCPPerceptionConnector(engine=self, port=port)
...
class _MCPPerceptionConnector(BaseMCPConnector):
    def __init__(self, engine: PerceptionEngine, port: int = 8783):
        super().__init__("PyRoboVision", port=port)
        self.engine = engine

    def get_mcp_tools(self) -> Dict[str, Any]:
        from pyrobovision._mcp_tools import PyRoboVisionMCPTools
        return PyRoboVisionMCPTools.get_tools()

    def get_tool_handlers(self) -> Any:
        from pyrobovision._mcp_tools import PyRoboVisionMCPHandler
        return PyRoboVisionMCPHandler(self.engine)
